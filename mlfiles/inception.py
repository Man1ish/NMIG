...
import sys
import warnings  # ✅ FIX: moved before usage
import psutil
import json
import os
...
import argparse
warnings.filterwarnings("ignore")  # ✅ Now safe to use
...
def main(args):
    try:
        ...
        if local:
            include_path = ""
        model_path = os.path.join(script_dir, include_path + "inception_v3.pth")
        ...
        if no_of_gpu > 1:
            batch_size = no_of_gpu * batch_size  # or set batch_size = no_of_gpu * N for heavier workloads
            input_batch = input_batch.repeat(batch_size, 1, 1, 1)  # Repeat along batch dimension
        ...
        _, indices = torch.max(output, 1)  # indices: [batch_size]

        # Apply softmax across all outputs
        probs = torch.nn.functional.softmax(output, dim=1)  # probs: [batch_size, num_classes]
